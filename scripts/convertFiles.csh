...
if ($#argv < 2) then
   echo "Usage: $0 <simulation name> <time1> <time2>"
   goto done
endif
...
if (! -d $sim/MS) then
  echo Simulation dump directory not present for $sim. Exiting..
  exit 1
endif
  ...
  foreach spec ($specs)
    if($type == DENSITY) then
      ...
      if($#stypes == 0) then
        ...
        if( -e $file ) then
          hdf2root $file ${sim}/Data/$spec-${stime}.root
        endif 
      else 
        foreach stype ($stypes)
          set file = ${sim}/MS/$type/$spec/$stype/$stype-$spec-${stime}.h5
          if( -e $file ) then
            hdf2root $file ${sim}/Data/$stype-$spec-${stime}.root
          endif
        end
      endif
    else
      ...
      if($type == RAW) then
        ...
        if(-e $file) then
           hdf2root --seq $file ${sim}/Data/$type-$spec-${stime}.root
        endif
      else
        set file = ${sim}/MS/$type/$spec/$spec-${stime}.h5
        if(-e $file) then
