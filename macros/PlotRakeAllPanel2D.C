...
void PlotRakeAllPanel2D( const TString &sim, Int_t time, Float_t zoom=1, Int_t Nbins=1, const TString &options="") {
  ...
  hIonProb2D->Reset();
  {
	...
	if(integral<1) {
	  ...
	  hIonProb2D->SetBinContent(k,j,0.0);	  
	}
  ...
  hIonProb1D->Reset();
  {
	...
	hIonProb1D->SetBinContent(j,0.0);	  
      }
  ...
  if(opt.Contains("units") && n0) {
	...
	hB2D[2]->SetBins(NbinsX,xMin,xMax,NbinsY,ymin,ymax);
	...
	for(Int_t j=0;j<=NbinsX;j++) {
	  for(Int_t k=0;k<=NbinsY;k++) {
	    ...
	    hB2D[2]->SetBinContent(j,k,hB2D[2]->GetBinContent(j,k) * ( E0 / (PUnits::GV/PUnits::m) ) );   
	  }
	...
	hB2D[2]->GetZaxis()->SetTitle("B_{x} [GV/m]");
  ...
  // Canvas setup
  TCanvas *C = new TCanvas("C","Rake 5 panel",1680,1050);
  ...
  // Output file
  TString fOutName = Form("./%s/Plots/RakeAllPanel2D/RakeAllPanel2D",pData->GetPath().c_str());
  fOutName += Form("-%s_%i",pData->GetName(),time);

  // Setup Pad layout:
  const Int_t Nx = 2;
  const Int_t Ny = 3;
  const Int_t NPad = Nx*Ny;
  TPad *pad[Nx][Ny]; 
  TH2F *hFrame[Nx][Ny];
  TString sLabels[] = {"(f)","(e)","(d)","(c)","(b)","(a)"};
  // Text objects
  ...
  Float_t vSpacing = 0.00;
  PGlobals::CanvasDoublePartition(C,Nx,Ny,lMargin,rMargin,bMargin,tMargin,vSpacing);
  
  // Define the frames for plotting
  Int_t fonttype = 43;
  Int_t fontsize = 40;
  Int_t tfontsize = 44;
  Float_t txoffset = 2.6;
  ...
  for(Int_t i=0;i<Nx;i++) {
    for(Int_t j=0;j<Ny;j++) {
      ...
      pad[i][j] = (TPad*) gROOT->FindObject(name);
      pad[i][j]->SetFrameLineWidth(2);  
      pad[i][j]->SetTickx(1);
      pad[i][j]->SetTicky(0);

      sprintf(name,"hFrame_%i_%i",i,j);
      hFrame[i][j] = (TH2F*) gROOT->FindObject(name);
      if(hFrame[i][j]) delete hFrame[i][j];
      hFrame[i][j] = (TH2F*) hDen2D[0]->Clone(name);
      hFrame[i][j]->Reset();
      ...
      // Format for y axis
      hFrame[i][j]->GetYaxis()->SetTitleFont(fonttype);
      hFrame[i][j]->GetYaxis()->SetTitleSize(tfontsize);
      hFrame[i][j]->GetYaxis()->SetTitleOffset(tyoffset);
      hFrame[i][j]->GetYaxis()->SetLabelFont(fonttype);
      hFrame[i][j]->GetYaxis()->SetLabelSize(fontsize);
      hFrame[i][j]->GetYaxis()->SetLabelOffset(lyoffset);

      hFrame[i][j]->GetYaxis()->SetTickLength(xFactor*tylength/yFactor);

      // Format for x axis
      hFrame[i][j]->GetXaxis()->SetTitleFont(fonttype);
      hFrame[i][j]->GetXaxis()->SetTitleSize(tfontsize+8);
      hFrame[i][j]->GetXaxis()->SetTitleOffset(txoffset);
      hFrame[i][j]->GetXaxis()->SetLabelFont(fonttype);
      hFrame[i][j]->GetXaxis()->SetLabelSize(fontsize+4);
      hFrame[i][j]->GetXaxis()->SetLabelOffset(lxoffset);
    
      hFrame[i][j]->GetXaxis()->SetTickLength(yFactor*txlength/xFactor);      
  ...
  pad[0][2]->Update();
  y1 = pad[0][2]->GetBottomMargin();
  y2 = 1 - pad[0][2]->GetTopMargin();
  x1 = pad[0][2]->GetLeftMargin();
  x2 = 1 - pad[0][2]->GetRightMargin();
  ...
  Float_t yaxismin  =  pad[0][2]->GetUymin();
  Float_t yaxismax  =  pad[0][2]->GetUymin() + 0.33*(pad[0][2]->GetUymax() - pad[0][2]->GetUymin()) - 0.00;
  ...
  textLabel[5] = new TPaveText(xMax - 0.07 * xRange, yMax - 0.2 * yRange, xMax - 0.01 * xRange, yMax - 0.01 * yRange);
  PGlobals::SetPaveTextStyle(textLabel[5],12); 
  textLabel[5]->SetTextFont(42);
  textLabel[5]->AddText(sLabels[5]);
  textLabel[5]->Draw();
  ...
  y1 = pad[0][1]->GetBottomMargin();
  y2 = 1 - pad[0][1]->GetTopMargin();
  x1 = pad[0][1]->GetLeftMargin();
  x2 = 1 - pad[0][1]->GetRightMargin();
  ...
  textLabel[4] = new TPaveText(xMax - 0.07 * xRange, yMax - 0.2 * yRange, xMax - 0.01 * xRange, yMax - 0.01 * yRange);
  ...
  // hE1D[1]->SetLineColor(kOrange+10);

  // hE1D[1]->Draw("sameL");
  
  pad[0][0]->Update();
  
  y1 = pad[0][0]->GetBottomMargin();
  y2 = 1 - pad[0][0]->GetTopMargin();
  x1 = pad[0][0]->GetLeftMargin();
  x2 = 1 - pad[0][0]->GetRightMargin();
  ...
  textLabel[3] = new TPaveText(xMax - 0.07 * xRange, yMax - 0.2 * yRange, xMax - 0.01 * xRange, yMax - 0.01 * yRange);
  ...
  xFactor = pad[0][0]->GetAbsWNDC()/pad[1][2]->GetAbsWNDC();
  yFactor = pad[0][0]->GetAbsHNDC()/pad[1][2]->GetAbsHNDC();
  ...
  y1 = pad[1][2]->GetBottomMargin();
  y2 = 1 - pad[1][2]->GetTopMargin();
  x1 = pad[1][2]->GetLeftMargin();
  x2 = 1 - pad[1][2]->GetRightMargin();
  ...
  textLabel[2] = new TPaveText(xMax - 0.07 * xRange, yMax - 0.2 * yRange, xMax - 0.01 * xRange, yMax - 0.01 * yRange);
  ...
  y1 = pad[1][1]->GetBottomMargin();
  y2 = 1 - pad[1][1]->GetTopMargin();
  x1 = pad[1][1]->GetLeftMargin();
  x2 = 1 - pad[1][1]->GetRightMargin();
  ...
  textLabel[1] = new TPaveText(xMax - 0.07 * xRange, yMax - 0.2 * yRange, xMax - 0.01 * xRange, yMax - 0.01 * yRange);
  ...
  y1 = pad[1][0]->GetBottomMargin();
  y2 = 1 - pad[1][0]->GetTopMargin();
  x1 = pad[1][0]->GetLeftMargin();
  x2 = 1 - pad[1][0]->GetRightMargin();
  ...
  textLabel[0] = new TPaveText(xMax - 0.07 * xRange, yMax - 0.2 * yRange, xMax - 0.01 * xRange, yMax - 0.01 * yRange);
