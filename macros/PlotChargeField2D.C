...
void PlotChargeField2D( const TString &sim, Int_t time, Float_t zoom=2, Int_t Nbins=2, const TString &options="") {
  ...
  // Refresh Canvases and set the Style
  PGlobals::Initialize();
  ...
  for(Int_t i=0;i<Nspecies;i++) {
    ...
    if(!hDen2D[i]) {
      cout << Form(" Charge density of specie %i not found: \n %s",i,pData->GetChargeFileName(i)->c_str())<< endl;
    }
  ...
  for(Int_t i=0;i<Nfields;i++) {
    ...
    if(!pData->GetEfieldFileName(i)) {
      cout << Form(" No longitudinal E field found. Exiting... ") << endl;
      return;
    }
  ...
  if(opt.Contains("curr")) {
	...
	axisb->SetLineColor(kGray+3);//PGlobals::elecLine);
	axisb->SetLabelColor(kGray+3);//PGlobals::elecLine);
	axisb->SetLabelSize(0.06);
	axisb->SetLabelOffset(0.01);
	axisb->SetLabelFont(42);
	axisb->SetTitleColor(kGray+3);//PGlobals::elecLine);
  ...
  TPaveText *textTime = new TPaveText(xMax - 0.30*xRange, yMax-0.15*yRange, xMax-0.05*xRange, yMax-0.05*yRange);
  ...
  hV1D[0]->SetLineColor(PGlobals::elecLine);
  ...
  // 			      (trapPotential)*slope);
  // lineTrap->SetLineColor(PGlobals::elecLine);
