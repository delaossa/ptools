...
void PlotEvolutions(const TString &sim, const TString &options="png") { 
  ...
  // Dynamic Ey palette
  const Int_t eyPNRGBs = 6;
  const Int_t eyPNCont = 64;
  Float_t zeroPos = -Emin/(Emax-Emin);
  
  Double_t eyPStops[eyPNRGBs] = { 0.00, zeroPos-3.0/eyPNCont,zeroPos-1.0/eyPNCont, zeroPos, zeroPos+1.0/eyPNCont, 1.00 };
  Double_t eyPRed[eyPNRGBs]   = { 0.106, 0.698, 0.90, 0.90, 0.965, 0.518 };
  Double_t eyPGreen[eyPNRGBs] = { 0.078, 0.818, 0.90, 0.90, 0.925, 0.078 };
  Double_t eyPBlue[eyPNRGBs]  = { 0.518, 0.880, 0.90, 0.90, 0.353, 0.106 };
   
  PPalette * eyPalette = (PPalette*) gROOT->FindObject("rbowwhite");
  eyPalette->CreateGradientColorTable(eyPNRGBs, eyPStops, 
				      eyPRed, eyPGreen, eyPBlue, eyPNCont);
  ...
  // Print to a file
  // Output file
  fOutName = Form("./%s/Plots/Evolutions/Evolutions-Ey-%s",sim.Data(),sim.Data());
