...
void PlotRakeZoom2D( const TString &sim, Int_t time, Float_t zoom=2, Int_t Nbins=2, const TString &options="") {
  ...
  Float_t xMin = pData->GetXMin(0);
  Float_t xMax = pData->GetXMax(0);
  Float_t xRange = xMax - xMin;  
  pData->SetX1Min(xMin);
  pData->SetX1Max(xMax);

  //  cout << Form("Plot ranges:  {%f,%f}  {%f,%f}",xMin,xMax,yMin,yMax) << endl;
  ...
  for(Int_t i=0;i<Nspecies;i++) {
    ...
    if(!pData->Is3D())
      hDen2D[i] = pData->GetH2(pData->GetChargeFileName(i)->c_str(),"charge",opt);
    else
  ...
  for(Int_t i=0;i<Nfields;i++) {
    ...
    if(!pData->Is3D())
      hE2D[i] = pData->GetH2(pData->GetEfieldFileName(i)->c_str(),enam,opt);
    else
      hE2D[i] = pData->GetH2SliceZYrange(pData->GetEfieldFileName(i)->c_str(),enam,-1,Nbins,opt+"avg");
    ...
    if(pData->Is3D()) {
      ...
      if(i==0) 
	hE1D[i] = pData->GetH1SliceZ3D(pData->GetEfieldFileName(i)->c_str(),enam,-1,Nbins,-1,Nbins,opt+"avg");
      else  
	hE1D[i] = pData->GetH1SliceZ3D(pData->GetEfieldFileName(i)->c_str(),enam,-Nbins,Nbins,-Nbins,Nbins,opt+"avg");
      
    } else if(pData->IsCyl()) { // Cylindrical: The first bin with r>0 is actually the number 1 (not the 0).
      
      hE1D[i] = pData->GetH1SliceZ(pData->GetEfieldFileName(i)->c_str(),enam,1,Nbins,opt+"avg");
	...
	hE1D[i] = pData->GetH1SliceZ(pData->GetEfieldFileName(i)->c_str(),enam,-Nbins,Nbins,opt+"avg");    
    }
  ...
  cout << Form("  -->  10%% ionization found at z = %.2f",z10) << endl;
  cout << Form("  --> 100%% ionization found at z = %.2f",z100) << endl;
  ...
  for(Int_t ip=hE1D[0]->GetNbinsX()-10;ip>1;ip--) {
    ...
    if(E1*E2 < 0) { // change of sign means a crossing!
      ...
      if(Ncross>0 && fabs(Ecross[Ncross-1]-zcross)<TMath::PiOver2() ) continue;	
      // cout << " CROSS! " << endl;
  ...
  if(opt.Contains("units") && n0) {
    ...
    xRange *= skindepth / PUnits::um;
    yMin *= skindepth / PUnits::um;
    yMax *= skindepth / PUnits::um;
    yRange *= skindepth / PUnits::um;
  ...
  TClonesArray graphsI2D_main("TGraph",NcontI);
  {
	...
	if( i==0 ) {
	  ...
	  if(j==0) {
	    gr2->SetLineWidth(2);
	    gr2->SetLineStyle(2);
	    //	    gr2->SetLineColor(PlasmaGlob::elecLine);
  ...
  // Canvas setup
  TCanvas *C = new TCanvas("C","2D Charge density and Electric field",1024,768);
  ...
  // Output file
  TString fOutName = Form("./%s/Plots/RakeZoom2D/RakeZoom2D",pData->GetPath().c_str());
  ...
  // zoom in x
  xMin = -9.0 + pData->Shift("centercomov");
  xMax = -3.77 + pData->Shift("centercomov");
  xMin -= shiftz; 
  xMax -= shiftz; 
  if(opt.Contains("units")) {
    xMin *= skindepth / PUnits::um;
  ...
  hDen2D[0]->GetXaxis()->SetRangeUser(xMin,xMax);
  ...
  hFrame->GetYaxis()->SetTitleSize(0.095);
  ...
  hFrame = (TH2F*) gROOT->FindObject("hFrame5");
  if(hFrame) delete hFrame;
  hFrame = (TH2F*) hDen2D[0]->Clone("hFrame5");
  hFrame->Reset();

  Float_t yFactor = pad[0]->GetAbsHNDC()/pad[1]->GetAbsHNDC();
  
  
  hFrame->GetYaxis()->SetLabelSize(yFactor*0.09);
  hFrame->GetYaxis()->SetLabelOffset(0.02/yFactor);
  hFrame->GetYaxis()->SetTitleSize(yFactor*0.095);
  ...
  if(palette) {
    ...
    palette->SetX2NDC(x2 + 0.03);
    // palette->SetTitleFont(42);
    palette->SetTitleSize(yFactor*0.085);
