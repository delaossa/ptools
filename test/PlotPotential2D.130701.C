...
void PlotPotential2D( const TString &sim, Int_t time, Int_t zoom=2, Int_t Nbins=2, const TString &options="") {
  ...
  hIonProb2D->Reset();
  {
	...
	Double_t IonProb = (PFunc::ADK(E,Eion0,Z,l,m)/PUnits::atomictime)*(pData->GetDX(0)*timedepth);
  ...
  hIonProb2D->GetZaxis()->SetTitle("W_{ADK} [#omega_{p}]");
  ...
  if(opt.Contains("units") && n0) {
	...
	hV2D->GetZaxis()->SetTitle("#Psi [MV/e]");
	hV1D->GetYaxis()->SetTitle("#Psi [MV/e]");
  ...
  const Int_t potentialDNRGBs = 4;
  const Int_t potentialDNCont = TMath::Nint(10.0*(Vmax-Vmin)/trapPotential);
  Float_t zeroPos = (10.0)/potentialDNCont;

  Double_t potentialDStops[potentialDNRGBs] = { 0.00,  zeroPos-(1.0/potentialDNCont), zeroPos, 1.00 };
  Double_t potentialDRed[potentialDNRGBs]   = { 0.518, 0.965, 0.498, 0.106 };
  Double_t potentialDGreen[potentialDNRGBs] = { 0.078, 0.925, 0.718, 0.078 };
  Double_t potentialDBlue[potentialDNRGBs]  = { 0.106, 0.353, 0.780, 0.518 };
   
  PPalette * potentialPalette = (PPalette*) gROOT->FindObject("rbow2inv");
  potentialPalette->CreateGradientColorTable(potentialDNRGBs, potentialDStops, 
					     potentialDRed, potentialDGreen, potentialDBlue, potentialDNCont);
  ...
  hIonProb2D->GetZaxis()->SetTitleFont(42);
  hIonProb2D->GetZaxis()->SetTickLength(0.02/yFactor);

  //exFieldT->Draw();
  exIonP->Draw();
  
  hIonProb2D->GetZaxis()->SetRangeUser(0.0001,0.9999);

  hIonProb2D->Draw("colz same");

  Int_t Ncont = 1;
  TH2F * hIonProb2Dc = (TH2F*) hIonProb2D->Clone("hIonProb2Dc");
  hIonProb2Dc->GetZaxis()->SetRangeUser(0.01,1.0);
  hIonProb2Dc->SetContour(Ncont);
  hIonProb2Dc->SetLineColor(kGray+3);
  hIonProb2Dc->Draw("cont2 same");

  // Ncont = 5;
  // TH2F * hIonProb2Dc2 = (TH2F*) hIonProb2D->Clone("hIonProb2Dc2");

  // hIonProb2Dc2->SetContour(Ncont);
  // hIonProb2Dc2->SetLineColor(kGray+3);
  // hIonProb2Dc2->Draw("cont2 same");
  ...
  if(hFrame) delete hFrame;
  hFrame = (TH2F*) hDen2D[0]->Clone("hFrame3");
  hFrame->Reset();
  ...
  Float_t rightmin = -trapPotential;
  Float_t rightmax = hV1D->GetMaximum();

  hV2D->Draw("col z same");

  TH2F * hV2Dthin = (TH2F*) hV2D->Clone("hV2Dthin");
  Ncont = TMath::Nint(5.0*(Vmax-Vmin)/trapPotential);
  hV2Dthin->SetContour(Ncont);
  hV2Dthin->SetLineColor(kGray+1);
  hV2Dthin->Draw("cont2 same");
  
  TH2F * hV2Dthick = (TH2F*) hV2D->Clone("hV2Dthick");
  Ncont = TMath::Nint(1.0*(Vmax-Vmin)/trapPotential);
  hV2Dthick->SetContour(Ncont);
  hV2Dthick->SetLineColor(kGray+3);
  hV2Dthick->SetLineWidth(2);
  hV2Dthick->Draw("cont2 same");
 
  // Short excursion to pad 0
  // pad[0]->cd(); 
  // hV2Dthick->Draw("cont2 same"); 
  // pad[2]->cd();
  ...
  rightmin = -trapPotential;
  rightmax = hV1D->GetMaximum();
  Float_t slope = (yMax-yMin)/(rightmax-rightmin);
  
  for(Int_t j=0;j<hV1D->GetNbinsX();j++) {
    hV1D->SetBinContent(j+1,(hV1D->GetBinContent(j+1)-rightmin)*slope+yMin);
  }
  hV1D->SetLineStyle(1);
  hV1D->SetLineWidth(2);
  hV1D->SetLineColor(kGray+3);
