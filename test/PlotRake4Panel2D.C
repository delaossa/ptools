...
void PlotRake4Panel2D( const TString &sim, Int_t time, Double_t zoom=2, Int_t Nbins=2, const TString &options="") {
  ...
  // Canvas setup
  TCanvas *C = NULL;
  C = new TCanvas("C","2D Charge density and Electric field",1280,1640);
  ...
  // Output file
  TString fOutName = Form("./%s/Plots/Rake4Panel2D/Rake4Panel2D",pData->GetPath().c_str());
  ...
  pad[3]->Update();
  Float_t y1 = pad[3]->GetBottomMargin();
  Float_t y2 = 1 - pad[3]->GetTopMargin();
  Float_t x1 = pad[3]->GetLeftMargin();
  Float_t x2 = 1 - pad[3]->GetRightMargin();
  Float_t gap = 0.005;
  ...
  Float_t yaxismin  =  pad[3]->GetUymin();
  Float_t yaxismax  =  pad[3]->GetUymin() + 0.33*(pad[3]->GetUymax() - pad[3]->GetUymin()) - 0.00;
  ...
  if(opt.Contains("curr")) {
    ...
    axis->SetTitleSize(tfontsize-4);
    axis->SetTitleOffset(tyoffset-1);
    axis->SetLabelFont(fonttype);
    axis->SetLabelColor(kGray+3);//PlasmaGlob::elecLine);
    axis->SetLabelSize(fontsize-4);
    axis->SetLabelOffset(lyoffset-0.005);
    axis->SetTickSize(tylength);
    axis->SetTitle("I [kA]");
    axis->CenterTitle();
  ...
  textLabel[3] = new TPaveText(xMax - 0.08 * xRange, yMax - 0.2 * yRange, xMax - 0.02 * xRange, yMax - 0.05 * yRange);
  PlasmaGlob::SetPaveTextStyle(textLabel[3],12); 
  ...
  hETotal2D->GetZaxis()->SetTickLength(0.02);
  ...
  textLabel[2] = new TPaveText(xMax - 0.08 * xRange, yMax - 0.2 * yRange, xMax - 0.02 * xRange, yMax - 0.05 * yRange);
  ...
  hIonProb2D->GetZaxis()->SetTickLength(0.02);
  ...
  textLabel[1] = new TPaveText(xMax - 0.08 * xRange, yMax - 0.2 * yRange, xMax - 0.02 * xRange, yMax - 0.05 * yRange);
  ...
  hV2D->GetZaxis()->SetTickLength(0.02);
  ...
  textLabel[0] = new TPaveText(xMax - 0.08 * xRange, yMax - 0.2 * yRange, xMax - 0.02 * xRange, yMax - 0.05 * yRange);
