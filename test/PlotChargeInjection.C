...
int binary_search(Int_t tag[2], vector<Int_t*> tags, Int_t imin, Int_t imax) {
  
  // test if array is empty
  if (imax <= imin) { // set is empty, so return value showing not found
    // cout << Form("--> Tag NOT found!") << endl;
    return -1;
  } else {
    // calculate midpoint to cut set in half
    int imid = imin + ((imax - imin) / 2);
    
    // cout << Form(" tag = {%5i, %6i}  tags[%5i] = {%5i, %6i}  imin = %5i  imax = %5i",tag[0],tag[1],imid,tags[imid][0],tags[imid][1],imin,imax) << endl;
    
    // three-way comparison
    if(myComp(tag,tags[imid])) {
      // tag is in lower subset
      return binary_search(tag,tags,imin,imid-1);
    } else if (myComp(tags[imid],tag)) {
      // key is in upper subset
      return binary_search(tag,tags,imid+1, imax);
    } else {
      // key has been found
      // cout << Form("--> Tag found! at imid = %5i",imid) << endl;
      return imid;
    }
  }
}

void PlotChargeInjection( const TString &sim, Int_t zoom=2, Int_t Nbins=2, const TString &options="") {
  ...
  for(Int_t k=0;k<NFrames;k++) {
    ...
    zDenMaxTotal = new Float_t[Nspecies];
	...
	hDenTag2D[k] = (TH2F*) hDen2D[k][i]->Clone("hDenTag2D");
	hDenTag2D[k]->Reset();

	// We need to dump tagged electrons in selected slice.
	Float_t dx1 = hDen2D[k][i]->GetXaxis()->GetBinWidth(1);
	Float_t dx2 = hDen2D[k][i]->GetYaxis()->GetBinWidth(1);
	Float_t Dx3 = hDen2D[k][i]->GetYaxis()->GetBinWidth(1) * Nbins;
	// Volume element
	...
	// Search tags in the current time step:
	cout << " Searching.." << endl;
	...
	for(Int_t i=0;i<nentries;i++) {
	  ...
	  // Binary search for ordered vectors:
	  Int_t index = binary_search(tag,tags,0,(int)tags.size());
  ...
  for(Int_t k=0;k<NFrames;k++) {
    ...
    for(Int_t i=0;i<Nspecies;i++) {
      ...
      Float_t Max = zDenMaxTotal[1];
      
      // cout << Form(" Min = %f  Max = %f",Min,Max) << endl;
      
      hDen2D[k][i]->GetZaxis()->SetRangeUser(Min,Max);
    ...
    hDen2D[k][0]->GetZaxis()->SetRangeUser(Min,Max);
  ...
  // Output file
  TString fOutName = Form("./%s/Plots/ChargeInjection/ChargeInjection",pData->GetPath().c_str());
