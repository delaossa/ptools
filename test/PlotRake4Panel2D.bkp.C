...
void PlotRake4Panel2D( const TString &sim, Int_t time, Int_t zoom=2, Int_t Nbins=2, const TString &options="") {
  ...
  // Canvas setup
  TCanvas *C = NULL;
  if(opt.Contains("hres") && !opt.Contains("pdf")) 
    // high resolution for plain grahics output.
    C = new TCanvas("C","2D Charge density and Electric field",640,800);
  else 
  ...
  hFrame->GetYaxis()->SetLabelSize(0.095);
  ...
  hFrame->GetYaxis()->SetLabelSize(yFactor*0.095);
  ...
  if(palette) {
    ...
    palette->SetLabelSize(yFactor*0.09);
    palette->SetLabelFont(42);
  ...
  if(hFrame) delete hFrame;
  hFrame = (TH2F*) hDen2D[0]->Clone("hFrame4");
  hFrame->Reset();
  ...
  pad[3] = (TPad*) gROOT->FindObject(pname);
  pad[3]->Draw();
  pad[3]->cd(); // <--------------------------------------------------------- 4th panel
  pad[3]->SetFrameLineWidth(2);  
  pad[3]->SetTickx(1);
  ...
  yFactor = pad[0]->GetAbsHNDC()/pad[3]->GetAbsHNDC();
