...
void PlotRake3bPanel2D( const TString &sim, Int_t time, Float_t zoom=2, Int_t Nbins=2, const TString &options="") {
  ...
  TClonesArray graphsV2D_main("TGraph",Ncontours);
  {
    ...
    for(Int_t i = 0; i < ncontours; i++){
      ...
      // cout << Form(" Contour %i : %f",i,contours[i]) << endl;
    
      Int_t nj = clist->GetSize(); // number of curves for the i-th contour.
      for(Int_t j = 0 ; j<nj; j++) {
	gr = (TGraph*) clist->At(j);
	if(!gr) continue;
      
	// cout << Form(" Curve %i, mean = %f",j,gr->GetMean()) << endl;
  ...
  // Output file
  TString fOutName = Form("./%s/Plots/Rake3bPanel2D/Rake3bPanel2D",pData->GetPath().c_str());
  ...
  PlasmaGlob::SetPaveTextStyle(textTime,12); 
  char ctext[128];
  if(opt.Contains("units") && n0) 
