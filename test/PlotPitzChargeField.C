...
void PlotPitzChargeField( const TString &sim, Int_t time, Float_t zoom=2, Int_t Nbins=2, const TString &options="") {
  ...
  if(opt.Contains("units") && n0) {
	...
	hCur1D[i]->Scale(TMath::Abs(n0 * dV * (PConst::ElectronCharge/PUnits::picocoulomb) * (kp * PConst::c_light * PUnits::femtosecond *1000))); // in Ampere

	hCur1D[i]->GetYaxis()->SetTitle("I[A]");
	hCur1D[i]->GetYaxis()->SetTitle("");
	if(opt.Contains("comov"))
	  hCur1D[i]->GetXaxis()->SetTitle("#zeta [mm]");
	else
	  hCur1D[i]->GetXaxis()->SetTitle("z [mm]");
	...
	for(Int_t k=0;k<hE2D[i]->GetNbinsY();k++) {
	  hE2D[i]->SetBinContent(j,k, hE2D[i]->GetBinContent(j,k) * ( E0 / (PUnits::MV/PUnits::m) ) );
	}
	hE1D[i]->SetBinContent(j, hE1D[i]->GetBinContent(j) * ( E0 / (PUnits::MV) ) );
	hV1D[i]->SetBinContent(j, hV1D[i]->GetBinContent(j) * ( E0 / (PUnits::MV) ) );
      }
	...
	hE2D[i]->GetXaxis()->SetTitle("z [mm]");
      ...
      if(i==0)
	hE2D[i]->GetZaxis()->SetTitle("E_{z} [MV/m]");
      else if(i==1)
	hE2D[i]->GetZaxis()->SetTitle("E_{y} [MV/m]");
      else if(i==2)
	hE2D[i]->GetZaxis()->SetTitle("E_{x} [MV/m]");
	...
	hE1D[i]->GetXaxis()->SetTitle("z [mm]");
      ...
      if(i==0)
	hE1D[i]->GetYaxis()->SetTitle("E_{z} [MV/m]");
      else if(i==1)
	hE1D[i]->GetYaxis()->SetTitle("E_{y} [MV/m]");
      else if(i==2)
	hE1D[i]->GetYaxis()->SetTitle("E_{x} [MV/m]");
 ...
 // --------------------------------------------------- Vertical Zoom ------------
  ...
  if(opt.Contains("units")) {
    zStartPlasma *= skindepth / PUnits::mm;
    zStartNeutral *= skindepth / PUnits::mm;
    zEndNeutral *= skindepth / PUnits::mm;
  }
  ...
  if(Nspecies>2) {
	...
	(hE1D[0]->GetBinCenter(ibin1)-hE1D[0]->GetBinCenter(ibin2));
      ...
      cout << Form(" Slope at z bunch = %6.2f (MV/m)um ", ESlopeInjBeam) << endl ;
  ...
  // Output file
  TString fOutName = Form("./%s/Plots/PitzChargeField/PitzChargeField",pData->GetPath().c_str());
  ...
  if(opt.Contains("curr")) {
    ...
    cout << Form(" Maximum driver  current = %6.2f A ", curmax) << endl ;
    ...
    if(Nspecies>=3)
      if(hCur1D[2]) {
	cout << Form(" Maximum witness current = %6.2f A ", hCur1D[2]->GetMaximum()) << endl ;
  ...
  hFrame->GetYaxis()->SetLabelOffset(0.01/yFactor);
  hFrame->GetYaxis()->SetTitleSize(yFactor*0.1);
