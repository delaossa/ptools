...
void PlotRake3Panel2D( const TString &sim, Int_t time, Float_t zoom=2, Int_t Nbins=2, const TString &options="") {
  ...
  if(opt.Contains("hres") && !opt.Contains("pdf")) 
    ...
    C = new TCanvas("C","2D Charge density and Electric field",1280,1280);
  else 
    C = new TCanvas("C","2D Charge density and Electric field",640,640);
  ...
  // Output file
  TString fOutName = Form("./%s/Plots/Rake3Panel2D/Rake3Panel2D",pData->GetPath().c_str());
  ...
  Int_t fontsize = 18;
  Int_t tfontsize = 24;
  Float_t txoffset = 2.25;
  Float_t lxoffset = 0.02;
  Float_t tyoffset = 1.5;
  ...
  pad[2]->Update();
  Float_t y1 = pad[2]->GetBottomMargin();
  Float_t y2 = 1 - pad[2]->GetTopMargin();
  Float_t x1 = pad[2]->GetLeftMargin();
  Float_t x2 = 1 - pad[2]->GetRightMargin();
  Float_t gap = 0.005;
  ...
  Float_t yaxismax  =  pad[2]->GetUymin() + 0.33*(pad[2]->GetUymax() - pad[2]->GetUymin()) - 0.00;
  ...
  textLabel[2] = new TPaveText(xMin + 0.02 * xRange, yMax - 0.2 * yRange, xMin + 0.10 * xRange, yMax - 0.05 * yRange);
  ...
  textLabel[1] = new TPaveText(xMin + 0.02 * xRange, yMax - 0.2 * yRange, xMin + 0.10 * xRange, yMax - 0.05 * yRange);
  ...
  // hV2D[0]->GetZaxis()->SetNdivisions(505);
  ...
  textLabel[0] = new TPaveText(xMin + 0.02 * xRange, yMax - 0.2 * yRange, xMin + 0.10 * xRange, yMax - 0.05 * yRange);
