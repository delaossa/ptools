...
void DoEvolutions( const TString &sim, Int_t time, Int_t Nbins=1, const TString &options="") { 
  ...
  if(hDen2D) {
    ...
    if(gQvsTime==NULL) {
      ...
      gQvsTime->SetLineColor(PlasmaGlob::fieldLine);
      gQvsTime->SetMarkerStyle(20);
      gQvsTime->SetMarkerSize(0.4);
      gQvsTime->SetMarkerColor(PlasmaGlob::fieldLine);	
  ...
  if(hGvsZ) {
    ...
    // PlasmaGlob::SetH1Style((TH1*)gGvsZ,1);
    PlasmaGlob::SetGraphStyle(gGvsZ,1);
