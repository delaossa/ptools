...
void PlotFieldTotal2D( const TString &sim, Int_t time, Int_t zoom=2, Int_t Nbins=2, const TString &options="") {
  ...
  // Palettes setup
  TExec *exField  = new TExec("exField","redPalette->cd();");
  TExec *exProb   = new TExec("exProb","redPalette->cd();");
  ...
  // Output file
  TString fOutName = Form("./%s/Plots/FieldTotal2D/FieldTotal2D",sim.Data());
