...
void PlotRakePanel2D( const TString &sim, Int_t time, Int_t zoom=2, Int_t Nbins=2, const TString &options="") {
  ...
  hIonProb2D->GetZaxis()->SetRangeUser(1.01,99.99);
  ...
  // Canvas setup
  TCanvas *C = new TCanvas("C","2D Charge density and Electric field",858,1100);
  ...
  TExec *exIonP   = new TExec("exIonP","bluePalette->cd();");
  ...
  // Output file
  TString fOutName = Form("./%s/Plots/RakePanel2D/RakePanel2D",pData->GetPath().c_str());
  ...
  if(palette) {
    palette->SetY2NDC(y2 - 0.00);
    palette->SetY1NDC(0.5*(y1+y2) + 0.00);
    palette->SetX1NDC(x2 + 0.005);
    palette->SetX2NDC(x2 + 0.03);
    // palette->SetTitleFont(42);
    palette->SetTitleSize(yFactor*0.075);
  ...
  if(palette) {
    palette->SetY2NDC(0.5*(y1+y2) - 0.00);
    palette->SetY1NDC(y1);
    palette->SetX1NDC(x2 + 0.005);
    palette->SetX2NDC(x2 + 0.03);
    // palette->SetTitleFont(42);
  ...
  textLabel[3] = new TPaveText(xMin + 0.02*xRange, yMax-0.2*yRange, xMin+0.30*xRange, yMax-0.05*yRange);
