...
void DoSlice(Int_t Dim, Int_t &FirstBin, Int_t &LastBin) {
  ...
  // and then LastBin means the TOTAL width of the slice (in bin numbers).
    ...
    Int_t Pivote = midBin + abs(FirstBin);
    if(fabs(LastBin)>fabs(FirstBin)) LastBin = abs(FirstBin);
    FirstBin = Pivote + 1 - LastBin;
    LastBin  = Pivote;
...
void PData::LoadFileNames(Int_t t) {
  ...
  Float_t v = GetBeamVelocity();    
  if(v==0) v = 1.0; // If equals to 0 (default), then set to c.
  Float_t shiftz = v * GetRealTime();
  
  if(pParam.x1Min == -999.) {
    XMINR[0] = GetXMin(0);
  } else {
    XMINR[0] = pParam.x1Min + shiftz;
    cout << Form(" x1Min = %f",XMINR[0]) << endl;
  }
  if(pParam.x1Max == -999.) {
    XMAXR[0] = GetXMax(0);
  } else {
    XMAXR[0] = pParam.x1Max + shiftz;
...
void PData::GetBoxDimensionsFromFile(const char *filename) {
  ...
  //   cout << Form(" Dimension %i:  N = %i  XMIN = %f  XMIN = %f",i,NX[i],XMIN[i],XMAX[i]) << endl; 
...
//_______________________________________________________________________
Float_t PData::GetRealTimeFromFile(const char *filename) {
...
//_______________________________________________________________________
TH1F* PData::GetH1SliceZ(const char *filename,const char *dataname,Int_t FirstyBin, Int_t LastyBin, const char *options) {
  ...
  // The "int" option integrates the in the range of bin specified by
  // FirstyBin and LastyBin. 
  ...
  UInt_t zDim = dataDims[1];
  UInt_t yDim = dataDims[0];

  hsize_t  *count  = new hsize_t[rank];
  hsize_t  *offset = new hsize_t[rank];
  for(UInt_t k=0;k<rank;k++) {
    offset[k] = 0;
    count[k]  = dataDims[k];
  }
  
  // I don't know why, but this was crashing!!
  // Float_t **data = new Float_t*[yDim];
  // for (UInt_t i=0;i<yDim;i++) {
  //   data[i] = new Float_t[zDim];
  // }

  Float_t data[yDim][zDim];
  ...
  if(LastyBin>=(Int_t)yDim) LastyBin = yDim - 1;

  // Special slicing around the axis.
  if(FirstyBin<0) {
    DoSlice(yDim,FirstyBin,LastyBin);
  }
  
  if(LastyBin<FirstyBin) 
    LastyBin = FirstyBin;

  Float_t *zArray = new Float_t[zDim];
  memset(zArray,0,sizeof(Float_t)*zDim);

  // Sum the values 
  string sdata = dataname;
  Double_t y = 1;
  Double_t ybinsize = (ax2lims[1] - ax2lims[0])/yDim;
  // cout << Form("Nbins = %i, Min = %f, Max = %f -> ybinsize = %f",yDim,ax2lims[0],ax2lims[1],ybinsize) << endl;
  
  for(UInt_t i=0;i<zDim;i++) {
    for(Int_t j=FirstyBin;j<=LastyBin;j++) {
     
      if(opt.Contains("cyl") && (opt.Contains("sum") || opt.Contains("int") ) ) {
      	// For cylindrical coordinates, this is the radius in Osiris units:
	y = ybinsize * (j-0.5) + ax2lims[0];
      }
      
      if(sdata.find("charge") != string::npos || sdata.find("p1x1") != string::npos || sdata.find("p2x2") != string::npos) {
	// cout << Form(" (%i,%i) -> radius = %f , value = %9.6f",i,j,y,-data[j][i]) << endl;
    	zArray[i] += -data[j][i] * y;
      } else
	zArray[i] +=  data[j][i] * y;
    }
  ...
  // Histogram centering
  Float_t shiftz = Shift(opt);  
  Float_t zmin = ax1lims[0] - shiftz;
  Float_t zmax = ax1lims[1] - shiftz;
  
  
  TH1F* h1D = new TH1F();
  h1D->SetBins(zDim,zmin,zmax);
  
  for(UInt_t i=0;i<zDim;i++) {
    Double_t content = zArray[i];
    if(opt.Contains("avg")) content /= (LastyBin-FirstyBin+1);
    else if(opt.Contains("int")) content *= ybinsize;
...
//_______________________________________________________________________
TH1F* PData::GetH1SliceY(const char *filename,const char *dataname,Int_t FirstzBin, Int_t LastzBin, const char *options) {
  ...
  if(LastzBin>=(Int_t)zDim) LastzBin = zDim - 1;

  if(FirstzBin<0)
    DoSlice(zDim,FirstzBin,LastzBin);
  
  if(LastzBin<FirstzBin) 
    LastzBin = FirstzBin;
  
  Float_t *yArray = new Float_t[yDim];
  memset(yArray,0,sizeof(Float_t)*yDim);

  // Sum the values 
  string sdata = dataname;
  for(Int_t i=FirstzBin;i<=LastzBin;i++) {
    for(UInt_t j=0;j<yDim;j++) {
      if(sdata.find("charge") != string::npos || sdata.find("p1x1") != string::npos || sdata.find("p2x2") != string::npos)
	yArray[j] += -data[j][i];
      else
	yArray[j] += data[j][i];
  ...
  // Histogram range
  Float_t ymin = ax2lims[0];
  Float_t ymax = ax2lims[1];
  
  TH1F* h1D = new TH1F();
  h1D->SetBins(yDim,ymin,ymax);
  
  Double_t zbinsize = (ax1lims[1]-ax1lims[0])/zDim;
  for(UInt_t j=0;j<yDim;j++) {
    Double_t content = yArray[j];
    if(opt.Contains("avg")) content /= (LastzBin-FirstzBin+1);
    else if(opt.Contains("int")) content *= zbinsize;
...
UInt_t PData::GetRawArray(const char *filename, Float_t **var, const char *options) {
  ...
  // var = new Float_t*[Nvar]; This crashes.
...
void PData::GetH1Raw(const char *filename,const char *dataname, TH1F *h1D, const char *options) {
  ...
  for(UInt_t i=0;i<nDim;i++) {
    ...
    if(sdata.find("x1") != string::npos) data[i] -= shiftz;
...
void PData::GetH1RawCut(const char *filename,const char *dataname, 
  ...
  for(UInt_t i=0;i<nDim;i++) {
    ...
    if(scut.find("x1") != string::npos) cut[i] -= shiftz;
...
TH2F* PData::GetH2(const char *filename,const char *dataname, const char *options) {
  ...
  UInt_t zDim = dataDims[1];
  UInt_t yDim = dataDims[0];
  Float_t data[yDim][zDim];   // <-- Segmentation fault problem when yDim and zDim are too high.
  // Float_t **data = new Float_t*[yDim]; // <-- Segementation violation in every case.
  // for(UInt_t k=0;k<yDim;k++)
  //   data[k] = new Float_t[zDim];
  // memset(data,0,sizeof(Float_t)*yDim*zDim);

  DataSpace memspace(rank,dataDims);
  dataSet->read(data,PredType::NATIVE_FLOAT,memspace,dataSet->getSpace());
  dataSet->close();
  ...
  if(opt.Contains("center")) {
    Float_t kp = GetPlasmaK();
    if(opt.Contains("comov"))        // Centers on the head of the beam.
      shiftz += GetBeamStart()*kp;   // Centers on the start of the plasma channel.
    else
      shiftz += GetPlasmaStart()*kp;

    if(!opt.Contains("cyl"))
      shifty += (ax2lims[0] + ax2lims[1])/2.;
  }
  
  if(opt.Contains("comov")) {
    Float_t v = GetBeamVelocity();    
    if(v==0) v = 1.0; // If equals to 0 (default), then set to c.
    shiftz += v * GetRealTime();
  }   
  
  Float_t zmin = 0.01 * TMath::Nint( 100* (ax1lims[0] - shiftz));
  Float_t zmax = 0.01 * TMath::Nint( 100* (ax1lims[1] - shiftz));
  
  Float_t ymin = ax2lims[0] - shifty;
  Float_t ymax = ax2lims[1] - shifty;
 
  string sdata = dataname;
  TH2F *h2D = new TH2F();
  h2D->SetBins(zDim,zmin,zmax,yDim,ymin,ymax);
 
  for(UInt_t i=0;i<zDim;i++) {
    for(UInt_t j=0;j<yDim;j++) {
      
      // When dynamic memory assigment of data[j][i] above, the program crash when accessing data[0][0] !!
      // cout << Form("data[%i][%i] = ",j,i);
      // cout << Form("%f",data[j][i]) << endl;
      
      // if(sdata.find("charge") != string::npos || sdata.find("p1x1") != string::npos || sdata.find("p2x2") != string::npos)
      // 	h2D->SetBinContent(i+1,j+1,-data[j][i]);
      // else
      // 	h2D->SetBinContent(i+1,j+1,data[j][i]);
      if(sdata.find("charge") != string::npos || sdata.find("p1x1") != string::npos || sdata.find("p2x2") != string::npos)
      	h2D->SetBinContent(i,j,-data[j][i]);
      else
      	h2D->SetBinContent(i,j,data[j][i]);
...
void PData::GetH2Raw(const char *filename,const char *dataname1,const char *dataname2,TH2F *h2D,const char *options) {
  ...
  for(UInt_t i=0;i<nDim;i++) {
    ...
    if(sdata1.find("x1") != string::npos) data1[i] -= shiftz;
    if(sdata2.find("x1") != string::npos) data2[i] -= shiftz;
...
TH1F* PData::GetH1SliceZ3D(const char *filename,const char *dataname, 
			   Int_t FirstxBin, Int_t LastxBin, 
			   Int_t FirstyBin, Int_t LastyBin, const char *options) {
  ...
  UInt_t zDim = dataDims[2];
  UInt_t yDim = dataDims[1];
  UInt_t xDim = dataDims[0];
  ...
  if(LastxBin>=(Int_t)xDim) LastxBin = xDim - 1;

  if(FirstxBin<0)
    DoSlice(xDim,FirstxBin,LastxBin);
  ...
  Double_t xbinsize = (ax3lims[1] - ax3lims[0])/xDim;
  Double_t ybinsize = (ax2lims[1] - ax2lims[0])/yDim;

  TH1F *h1D = new TH1F(); 
  h1D->SetBins(zDim,zmin,zmax);
  for(UInt_t k=0;k<zDim;k++) {
    Double_t content = zArray[k];
    if(opt.Contains("avg")) content /= ((LastxBin-FirstxBin+1)*(LastyBin-FirstyBin+1));
    else if(opt.Contains("int")) content *= ybinsize * xbinsize;
...
//_______________________________________________________________________
TH1F* PData::GetH1SliceZ3Drange(const char *filename,const char *dataname, 
  ...
  //----------
  UInt_t x1Dim = GetX1N();
  Float_t x1Min = GetX1Min();
  Float_t x1Max = GetX1Max();
  ...
  Float_t   data[x3Dim][x2Dim];
  DataSpace memSpace(rank,count);

  Float_t zArray[x1Dim];
  // Set all elements to 0.
  memset(zArray,0,sizeof(Float_t)*x1Dim);
...
TH1F* PData::GetH1CylSliceZ3D(const char *filename,const char *dataname, 
  ...
  Int_t rBins[zDim];
  memset(rBins,0,sizeof(Int_t)*zDim);
 
  Float_t xmin = ax3lims[0];
  Float_t xmax = ax3lims[1];
  Float_t xbinsize = (xmax-xmin)/xDim;
  
  Float_t ymin = ax2lims[0];
  Float_t ymax = ax2lims[1];
  Float_t ybinsize = (ymax-ymin)/yDim;

  // This is the max value of r in the range specified.
  Float_t rmax =  ymin + (LastrBin+0.5)*ybinsize - (ymax+ymin)/2.;
  ...
  for(Int_t i=FirstrBin;i<=LastrBin;i++) {
    ...
    for(Int_t j=FirstrBin;j<LastrBin;j++) {
      ...
      Float_t y = ymin + (j+0.5)*ybinsize - (ymin+ymax)/2.;
...
//_______________________________________________________________________
TH2F* PData::GetH2SliceZY(const char *filename,const char *dataname, Int_t FirstxBin, Int_t LastxBin, const char *options) {
  ...
  Float_t yzArray[yDim][zDim];
  memset(yzArray,0,sizeof(Float_t)*yDim*zDim);
  ...
  for(UInt_t j=0;j<yDim;j++) 
    for(UInt_t k=0;k<zDim;k++) { 
      Double_t content = yzArray[j][k];
      if(opt.Contains("avg")) content /= (LastxBin-FirstxBin+1);
      else if(opt.Contains("int")) content *= xbinsize;
...
//_______________________________________________________________________
TH2F* PData::GetH2SliceZYrange(const char *filename,const char *dataname, Int_t Firstx3Bin, Int_t Lastx3Bin, const char *options) {
  ...
  UInt_t x2Dim = GetX2N();
  Float_t x2Min = GetX2Min();
  Float_t x2Max = GetX2Max();
  ...
  Float_t yzArray[x2Dim][x1Dim];
  memset(yzArray,0,sizeof(Float_t)*x2Dim*x1Dim);
   
  string sdata = dataname;
  // Sum the values 
  for(Int_t i=Firstx3Bin;i<=Lastx3Bin;i++) {
    dataSpace.selectHyperslab(H5S_SELECT_SET,count,offset);
    dataSet->read(data,PredType::NATIVE_FLOAT,memSpace,dataSpace);
...
//_______________________________________________________________________
TH2F* PData::GetH2SliceZX(const char *filename,const char *dataname,Int_t FirstyBin, Int_t LastyBin, const char *options) {
  ...
  Float_t xzArray[xDim][zDim];
  memset(xzArray,0,sizeof(Float_t)*xDim*zDim);
   
  string sdata = dataname;
  // Sum the values 
  offset[0] = 0;
  for(UInt_t i=0;i<xDim;i++) {
    dataSpace.selectHyperslab(H5S_SELECT_SET,count,offset);
    dataSet->read(data,type,memSpace,dataSpace);
  ...
  Float_t xmin = ax3lims[0] - shiftx;
  Float_t xmax = ax3lims[1] - shiftx;
  
  Double_t ybinsize = (ax2lims[1] - ax2lims[0])/yDim;
  
  TH2F *h2D = new TH2F(); 
  h2D->SetBins(zDim,zmin,zmax,xDim,xmin,xmax);
  for(UInt_t i=0;i<xDim;i++)  
    for(UInt_t k=0;k<zDim;k++) {
      Double_t content = xzArray[i][k];
      if(opt.Contains("avg")) content /= (LastyBin-FirstyBin+1);
      else if(opt.Contains("int")) content *= ybinsize;
      h2D->SetBinContent(k+1,i+1,content);
    }
...
//_______________________________________________________________________
TH2F* PData::GetH2SliceXY(const char *filename,const char *dataname, Int_t FirstzBin, Int_t LastzBin, const char *options) {
  ...
  hsize_t  *ax2Dims = new hsize_t[1];
  ax2->getSpace().getSimpleExtentDims(ax2Dims,NULL);
  DataSpace mem1(1,ax2Dims);
  ax2->read(ax2lims,PredType::NATIVE_DOUBLE,mem1,ax2->getSpace());
  ax2->close(); 
  delete ax2Dims;
  delete ax2;
  ...
  hsize_t  *ax3Dims = new hsize_t[1];
  ax3->getSpace().getSimpleExtentDims(ax3Dims,NULL);
  DataSpace mem2(1,ax3Dims);
  ax3->read(ax3lims,PredType::NATIVE_DOUBLE,mem2,ax3->getSpace());
  ax3->close();
  delete ax3Dims;
  delete ax3;
  ...
  Float_t xyArray[xDim][yDim];
  memset(xyArray,0,sizeof(Float_t)*xDim*yDim);
  ...
  TH2F* h2D = new TH2F();
  h2D->SetBins(xDim,ax3lims[0],ax3lims[1],yDim,ax2lims[0],ax2lims[1]);
  
  Double_t zbinsize = (ax1lims[1]-ax1lims[0])/zDim;
  for(UInt_t i=0;i<xDim;i++) 
    for(UInt_t j=0;j<yDim;j++) { 
      Double_t content = xyArray[i][j];
      if(opt.Contains("avg")) content /= (LastzBin-FirstzBin+1);
      else if(opt.Contains("int")) content *= zbinsize;
...
//_______________________________________________________________________
TH2F* PData::GetH2SliceXYrange(const char *filename,const char *dataname, Int_t Firstx1Bin, Int_t Lastx1Bin, const char *options) {
  ...
  UInt_t  x3Dim = GetX3N();
  Float_t x3Min = GetX3Min();
  Float_t x3Max = GetX3Max();
  ...
  Float_t yxArray[x3Dim][x2Dim];
  memset(yxArray,0,sizeof(Float_t)*x3Dim*x2Dim);
   
  string sdata = dataname;
  // Sum the values 
  for(Int_t i=Firstx1Bin;i<=Lastx1Bin;i++) {
    dataSpace.selectHyperslab(H5S_SELECT_SET,count,offset);
    dataSet->read(data,PredType::NATIVE_FLOAT,memSpace,dataSpace);
...
TH2F* PData::GetH2ZR(const char *filename,const char *dataname, const char *options) {
  ...
  Float_t rzArray[rDim][zDim];
  memset(rzArray,0,sizeof(Float_t)*rDim*zDim);
   
  Int_t rzBins[rDim][zDim];
  memset(rzBins,0,sizeof(Int_t)*rDim*zDim);
  ...
  for(UInt_t i=0;i<xDim;i++) {
    ...
    Float_t x = ax3lims[0] + (i+0.5)*(ax3lims[1]-ax3lims[0])/xDim - (ax3lims[1]+ax3lims[0])/2.;
    
    for(UInt_t j=0;j<yDim;j++) {
      
      // Coordinates of current bin center:
      Float_t y = ax2lims[0] + (j+0.5)*(ax2lims[1]-ax2lims[0])/yDim - (ax2lims[1]+ax2lims[0])/2.;
      Float_t r = sqrt(x*x + y*y);
  ...
  TH2F *h2D = new TH2F(); 
  h2D->SetBins(zDim,ax1lims[0],ax1lims[1],rDim,axrlims[0],axrlims[1]);
  
  for(UInt_t ir=0;ir<rDim;ir++) 
    for(UInt_t k=0;k<zDim;k++) {
      if(!opt.Contains("sum"))
	h2D->SetBinContent(k+1,ir+1,rzArray[ir][k]/rzBins[ir][k]);
      else
       	h2D->SetBinContent(k+1,ir+1,rzArray[ir][k]);
    }
...
TH3F* PData::GetH3(const char *filename,const char *dataname) {
  ...
  TH3F *h3D = new TH3F();
  h3D->SetBins(zDim,ax1lims[0],ax1lims[1],yDim,ax2lims[0],ax2lims[1],xDim,ax3lims[0],ax3lims[1]);
